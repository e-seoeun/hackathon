...
def main():
    ...
    if len(today_ra_u):
        ax.plot(today_ra_u, tr["today_dec"], "-", linewidth=2.5, alpha=0.8,
                transform=world, zorder=5, label="today", color="royalblue")
        ...
        mark_oriented_triangle_world(ax, today_ra_u, tr["today_dec"], world,
                                     size=12, color="royalblue", ahead_px=2, z=20)


    if len(prev_ra_u):
        ax.plot(prev_ra_u, tr["prev_dec"], "-", linewidth=2.5, alpha=0.8,
                transform=world, zorder=4, label="prev", color="red")
    ...
    if dv is not None:
        if is_finite(dv.get("today_dv")) and is_finite(dv.get("prev_dv")):
            ...
            lines.append(f"dv   {dv['today_dv']:.4f} / {dv['prev_dv']:.4f}  (Δ {dvper:+.1f}%)")
        elif is_finite(dv.get("today_dv")):
            lines.append(f"dv   {dv['today_dv']:.4f}  (deg/s)")

        if is_finite(dv.get("today_dm")) and is_finite(dv.get("prev_dm")):
            if is_finite(dv.get("dm_diff")):
                lines.append(f"dm   {dv['today_dm']:.1f} / {dv['prev_dm']:.1f}  (Δ {dv['dm_diff']:+.1f})")
            else:
                lines.append(f"dm   {dv['today_dm']:.1f} / {dv['prev_dm']:.1f}")
        elif is_finite(dv.get("today_dm")):
            lines.append(f"dm   {dv['today_dm']:.1f}")
